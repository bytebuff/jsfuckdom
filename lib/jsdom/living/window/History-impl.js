...
exports.implementation = class HistoryImpl {
  ...
  get length() {
    ...
    return this._window._jsfuckSessionHistory.length;
  }
  ...
  go(delta) {
    ...
    if (delta === 0) {
      ...
      this._window._jsfuckSessionHistory.traverseByDelta(delta);
    }
  }
  ...
  _sharedPushAndReplaceState(data, url, historyHandling) {
    ...
    if (historyHandling === "push") {
      this._window._jsfuckSessionHistory.removeAllEntriesAfterCurrentEntry();

      this._window._jsfuckSessionHistory.clearHistoryTraversalTasks();
      ...
      this._window._jsfuckSessionHistory.addEntryAfterCurrentEntry(newEntry);
      this._window._jsfuckSessionHistory.updateCurrentEntry(newEntry);
    } else {
      const { currentEntry } = this._window._jsfuckSessionHistory;
    ...
    this._document._latestEntry = this._window._jsfuckSessionHistory.currentEntry;
  }
};
