...
function navigateToFragment(window, newURL, flags) {
  ...
  window._jsfuckSessionHistory.clearHistoryTraversalTasks();
  ...
  if (!flags.replacement) {
    ...
    window._jsfuckSessionHistory.removeAllEntriesAfterCurrentEntry();
  }
  const newEntry = { document, url: newURL };
  window._jsfuckSessionHistory.addEntryAfterCurrentEntry(newEntry);
  window._jsfuckSessionHistory.traverseHistory(newEntry, { nonBlockingEvents: true, replacement: flags.replacement });
}
